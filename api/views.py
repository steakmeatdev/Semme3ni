...
class GetRoom(APIView):
    ...
    isauth_url_kwarg = "isauth"
    print("Got here")

    def get(self, request, format=None):
        code = request.GET.get(self.lookup_url_kwarg)
        isauth = request.GET.get(self.isauth_url_kwarg)
        if code != None:
...
class JoinRoom(APIView):
    def post(self, request, format=None):
        ...
        return Response(
            {"Bad request": "Post data missing"}, status=status.HTTP_400_BAD_REQUEST
        )
        ...
        if serializer.is_valid():
            ...
            if queryset.exists():
                ...
                if room_result.host != self.request.session.session_key:
                    ...
                    room_result.guest_can_pause = serializer.validated_data.get(
                        "guest_can_pause"
                    )
                    room_result.votes_to_skip = serializer.validated_data.get(
                        "votes_to_skip"
                    )
                    room_result.save(update_fields=["guest_can_pause", "votes_to_skip"])
                    return Response(
                        RoomSerializer(room_result).data, status=status.HTTP_200_OK
                    )
            else:
                return Response(
